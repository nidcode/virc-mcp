...
- [x] `observed` の文言や、審判不能を示す典型パターン（延期・中止・実施できず等）を
...
**対応（2026-09-07）**: `core/tools.ts` の `record_outcome` ハンドラに、
`status === 'refuted'` かつ `observed` が「延期・中止・キャンセル・順延・見送り・
未実施・欠場・中断・実施(でき|され)(ず|なかった)・開催(でき|され)(ず|なかった)/中止・
行(われ|え)なかった・DNS」等、行為そのものが不成立だったことを示す語にマッチした場合に
無条件で拒否するガード（`looksUnadjudicable`）を追加。`record_decision` の
`constraint_compliance` と同じ「曖昧な入力はまず拒否する」方針に揃え、警告つき通過や
上書きフラグは設けていない（拒否理由と `unadjudicable` への直し方をエラーメッセージに
明記）。汎用語の「できなかった」単体は含めず、「ペースを維持できなかった」のような
正当な refuted の誤検知を避けた。隔離フィクスチャで (1) 延期/中止文言を伴う refuted の
拒否、(2) 実測して外れた正当な refuted の成立、(3) unadjudicable がガードの影響を
受けないこと、を確認済み。
